...
:param location: Location of the UPnP description
		...
		#
			...
			#
				...
				services = ([ ] if (device == None) else device.service_get_unique_types())
				...
				#
					...
					if (self.rootdevice_is_known(device = identifier['device'])):
					#
						ssdp_request.reset_headers()
						ssdp_request.set_header("NTS", "ssdp:byebye")
			...
			#
				...
				ssdp_response.set_header("Date", RfcBasics.get_rfc1123_datetime(time()))
...
:since: v0.1.00
		...
		#
			...
			#
				...
				#
					Hooks.call("dNG.pas.upnp.ControlPoint.hostDeviceRemove", identifier = identifier)
				...
				#
					...
					if (usn_data['class'] == "device"): Hooks.call("dNG.pas.upnp.ControlPoint.deviceRemove", identifier = usn_data)
					Hooks.call("dNG.pas.upnp.ControlPoint.usnDelete", identifier = usn_data)
				#
...
:since: v0.1.00
		...
		if (self.log_handler != None): self.log_handler.debug("#echo(__FILEPATH__)# -ControlPoint.delete_usn({0}, bootid, configid, additional_data)- (#echo(__LINE__)#)".format(usn))
		...
		#
			...
			#
				...
				#
					...
					Hooks.call("dNG.pas.upnp.ControlPoint.hostDeviceAdd", identifier = device_identifier)
					...
					#
						...
						Hooks.call("dNG.pas.upnp.ControlPoint.hostDeviceAdd", identifier = embedded_device_identifier)
					#
				...
				#
					...
					#
						...
						#
							...
							Hooks.call("dNG.pas.upnp.ControlPoint.usnNew", identifier = self.usns[usn])
							if (self.usns[usn]['class'] == "device"): Hooks.call("dNG.pas.upnp.ControlPoint.deviceAdd", identifier = self.usns[usn])
						#
					#
				#
			#
...
:param usn: UPnP USN
	...
	def rootdevice_is_known(self, **kwargs):
	#
		"""
					...
					#
						...
						#
							...
							#
								...
								if (self.rootdevice_is_known(uuid = uuid)): results.append({ "usn": "uuid:{0}::upnp:rootdevice".format(device.get_udn()), "location": device.get_desc_url(), "search_target": "upnp:rootdevice" })
							#
						#
...
:since: v0.1.00
		...
		Hooks.call("dNG.pas.upnp.ControlPoint.startup")
...
:since: v0.1.00
		...
		Hooks.call("dNG.pas.upnp.ControlPoint.shutdown")
...
:since: v0.1.00
		...
		if (self.log_handler != None): self.log_handler.debug("#echo(__FILEPATH__)# -ControlPoint.update_usn({0}, {1}, bootid, bootid_old, configid, {2:d}, unicast_port, {3:.1f}, {4}, additional_data)- (#echo(__LINE__)#)".format(servername, usn, timeout, http_version, url))
